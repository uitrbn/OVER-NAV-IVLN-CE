...
class ObservationsDict(dict):
    def pin_memory(self):
        for k, v in self.items():
            if k not in ['keywords', 'rel_heading_feats', 'rel_distance_feats']:
                self[k] = v.pin_memory()
            else:
                for i in range(len(v)):
                    self[k][i] = v[i].pin_memory()
...
def collate_fn(batch):
    ...
    def _pad_keywords(t, sensor, max_len):
        if sensor == 'keywords':
            # keywords_token_feat_dim 
            feat_dim = t[0].shape[-1]
            assert feat_dim == 20
            t += ([np.zeros((0, feat_dim), dtype=np.int64)] * (max_len - len(t)))
            return t
        elif sensor == 'rel_heading_feats':
            feat_dim = t[0].shape[-1]
            assert feat_dim == 2
            t += ([np.zeros((0, feat_dim), dtype=np.float32)] * (max_len - len(t)))
            return t
        elif sensor == 'rel_distance_feats':
            feat_dim = t[0].shape[-1]
            assert feat_dim == 2
            t += ([np.zeros((0, feat_dim), dtype=np.float32)] * (max_len - len(t)))
            return t
    ...
    for bid in range(B):
        for sensor in observations_batch:
            if sensor not in ['keywords_map', 'keywords', 'rel_heading_feats', 'rel_distance_feats']:
                observations_batch[sensor][bid] = _pad_helper(
                    observations_batch[sensor][bid], max_traj_len, fill_val=1.0
                )
            else:
                observations_batch[sensor][bid] = _pad_keywords(observations_batch[sensor][bid], sensor, max_len=max_traj_len)
    ...
    for sensor in observations_batch:
        if sensor not in ['keywords_map', 'keywords', 'rel_heading_feats', 'rel_distance_feats']:
            ...
            new_obs_sensor = [observations_batch[sensor][i][j] for j in range(len(observations_batch[sensor][0])) for i in range(len(observations_batch[sensor]))]
            assert sensor != 'keywords_map'
            observations_batch[sensor] = new_obs_sensor
...
class IWTrajectoryDataset(torch.utils.data.IterableDataset):
    ...
    def __next__(self):
        ...
        # for k in ['keywords', 'rel_heading_feats']:
        #     del obs[k]
        for k, v in obs.items():
            if k in ['keywords', 'rel_heading_feats', 'keywords_map', 'rel_distance_feats']:
                obs[k] = v
            else:
...
class DaggerTrainer(BaseVLNCETrainer):
    def __init__(self, config=None):
        ...
        ) # data/trajectories_dirs/map_cma/pred_semantics/iterative_maps/trajectories.lmdb
    ...
    def _update_dataset(self, data_it):
        ...
        observations = envs.reset() # 这个observation是初始observation
        ...
        batch = batch_obs(observations, self.device) # Transpose a batch of observation dicts to a dict of batched observations.
    ...
    def train(self) -> None:
        ...
        self.config.defrost()
        eps = self.config.IL.DAGGER.expert_policy_sensor # SHORTEST_PATH_SENSOR
        if eps not in self.config.TASK_CONFIG.TASK.SENSORS: # false
        ...
        # observation_space: Dict('depth': Box(0.0, 1.0, (256, 256, 1), float32), 'instruction': Discrete(1), 'progress': Box(0.0, 1.0, (1,), float64), 'rgb': Box(0, 255, (224, 224, 3), uint8), 'shortest_path_sensor': Box(0.0, 100.0, (1,), float64), 'occupancy_map': Box(0, 255, (64, 64), uint8), 'semantic_map': Box(0, 255, (64, 64), uint8))
        ...
        with TensorboardWriter(
            ...
            for dagger_it in range(self.config.IL.DAGGER.iterations): # 1

                step_id = 0
                if not self.config.IL.DAGGER.preload_lmdb_features: # true
                    if 'UPDATE_DATASET_ENVIRONMENTS' in self.config:
                        self.config.defrost()
                        old_num_environment = self.config.NUM_ENVIRONMENTS
                        self.config.NUM_ENVIRONMENTS = self.config.UPDATE_DATASET_ENVIRONMENTS
                        self._update_dataset(
                            dagger_it + (1 if self.config.IL.load_from_ckpt else 0)
                        )
                        self.config.NUM_ENVIRONMENTS = old_num_environment
                        self.config.freeze()
                    else:
                        self._update_dataset(
                            dagger_it + (1 if self.config.IL.load_from_ckpt else 0)
                    ...
                    for batch in tqdm.tqdm(
                        ...
                        observations_batch = {
                            k: v.to(
                                ...
                                dtype=torch.float32,
                                non_blocking=True,
                            ) if k not in ['keywords', 'keywords_map', 'rel_heading_feats', 'rel_distance_feats'] else v
