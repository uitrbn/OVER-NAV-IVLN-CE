...
from ivlnce_baselines.common.mapping_module.setup_mapping_module import (
    ...
    setup_keywords_inputs_from_obs_dict,
)
...
from ivlnce_baselines.common.mapping_module.keywords_mapper3 import (
    KeywordsMappingModule3
)
from ivlnce_baselines.common.mapping_module.keywords_mapper5 import (
    KeywordsMappingModule5
)
from ivlnce_baselines.common.mapping_module.keywords_mapper6 import (
    KeywordsMappingModule6
)
...
class Mapper(ObservationTransformer):
    def __init__(
        ...
        self.keywords_mapping_module = None
        self.split = split
    ...
    def forward(self, observations: Dict[str, Tensor]) -> Dict[str, Tensor]:
        ...
        if self.keywords_map:
            self.setup_keywords_mapping_module(observations, self.split)
            observations = self.update_keywords_maps_from_observations(observations)
            observations = self.delete_extra_information_keywords(observations)
        
        observations = self.delete_extra_information(observations)
        return observations
    
    def update_keywords_maps_from_observations(self, observations):
        (
            episodes_info,
            input_observations,
            robot_current_state,
        ) = setup_keywords_inputs_from_obs_dict(observations)

        generated_keywords_map = self.keywords_mapping_module(
            episodes_info,
            input_observations,
            robot_current_state,
        )

        observations['keywords_map'] = generated_keywords_map
        return observations
    
    def delete_extra_information_keywords(self, observations):
        return observations
    ...
    def from_config(cls, config: Config, visualize=False):
        ...
        return cls(
            ...
            keywords_map='KEYWORDS_MAP' in config and config.KEYWORDS_MAP,
            split=config.DETECTION_DB_SPLIT if 'DETECTION_DB_SPLIT' in config else 'train',
        )
...
@baseline_registry.register_obs_transformer()
class KeywordsPredictedSemanticsIterativeMapper3(Mapper):
    def setup_mapping_module(self, observations: Dict[str, Tensor]):
        if self.mapping_module is None:
            self.mapping_module = create_predicted_semantics_iterative_mapper(
                device=observations["depth"].device,
    ...
    def setup_keywords_mapping_module(self, observations: Dict[str, Tensor], split: str):
        if self.keywords_map and self.keywords_mapping_module is None:
            print('initialized keywords mapping module 3')
            self.keywords_mapping_module = KeywordsMappingModule3(split=split)

@baseline_registry.register_obs_transformer()
class KeywordsPredictedSemanticsIterativeMapper5(Mapper):
    def setup_mapping_module(self, observations: Dict[str, Tensor]):
        if self.mapping_module is None:
            self.mapping_module = create_predicted_semantics_iterative_mapper(
                device=observations["depth"].device,
    ...
    def setup_keywords_mapping_module(self, observations: Dict[str, Tensor], split: str):
        if self.keywords_map and self.keywords_mapping_module is None:
            print('initialized keywords mapping module 5')
            self.keywords_mapping_module = KeywordsMappingModule5(split=split)

@baseline_registry.register_obs_transformer()
class KeywordsPredictedSemanticsIterativeMapper6(Mapper):
    def setup_mapping_module(self, observations: Dict[str, Tensor]):
        if self.mapping_module is None:
            self.mapping_module = create_predicted_semantics_iterative_mapper(
                device=observations["depth"].device,
    ...
    def setup_keywords_mapping_module(self, observations: Dict[str, Tensor], split: str):
        if self.keywords_map and self.keywords_mapping_module is None:
            print('initialized keywords mapping module 6')
            self.keywords_mapping_module = KeywordsMappingModule6(split=split)
