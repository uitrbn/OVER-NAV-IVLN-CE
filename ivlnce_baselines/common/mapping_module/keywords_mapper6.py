...
class KeywordsMappingModule6:
    def __init__(self, split='train'):
        ...
        self.counter = 0
        print('Start reading train memory from db')
        self.read_train_memory_from_db()
    ...
    def update_memory(self, episode_info, observations, robot_state, disable=False):
    ...
    def clear_memory_by_scan(self, episodes_info):
        # assert len(episodes_info.env_names) == 11
        clear_scans = list()
        ...
        for finished_i in episodes_info.finished_indices():
            ...
            self.detection_result[scan] = defaultdict(dict)
            print('clear memory by scan: ', scan)
            clear_scans.append(scan)
            self.counter += 1
            # assert self.counter <= 11, 'counter should be less than 11'
    ...
    def read_train_memory_from_db(self, read_scan=None):
        if not hasattr(self, 'train_predetect_result_db'):
            self.train_predetect_db_path = 'R2R_{}_enc_detection_owlvit-large-patch14.ivlnce.db'.format('train')
            self.train_predetect_result_db = pickledb.load(self.train_predetect_db_path, False)
            self.train_predetect_keys = self.train_predetect_result_db.getall()

        for i, key in enumerate(self.train_predetect_keys):
            if i % 10000 == 0:
                print('read train memory from db: {}/{}'.format(i, len(self.train_predetect_keys)))
            scan, viewpoint, inst_id, ix = key.split('%')
            if read_scan is not None and scan != read_scan:
                continue
            result = self.train_predetect_result_db.get(key)
            if not result:
                continue
    ...
    def __call__(
        ...
        clear_scans = self.clear_memory_by_scan(episodes_info)
        for scan in clear_scans:
            self.read_train_memory_from_db(read_scan=scan)
        # 1. update memory
