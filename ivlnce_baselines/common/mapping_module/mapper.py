...
@dataclass
class Observations:
    semantics: torch.LongTensor # torch.Size([2, 3, 224, 224])
    depth_normalized: torch.FloatTensor # torch.Size([2, 1, 256, 256])
    rgb: torch.FloatTensor # torch.Size([2, 1, 256, 256])
    # predicted_semantics: torch.LongTensor = field(init=False, default=None)


@dataclass
class Pointcloud:
    batch_indices: torch.LongTensor = None # [point number, ]
    xyz: torch.FloatTensor = None # [point number, 3]
...
class SemanticPointcloud(Pointcloud):
    ...
    semantics: torch.LongTensor = None # [point number, ]
...
class GenerateSemanticPointCloud(nn.Module):
    ...
    def forward(
        ...
        semantics_flat = linearize(observations.semantics) # observations.semantics的shape是[2, 1, 256, 256]
...
class UpdateWorldSemanticPointcloud(AbstractUpdateWorldSemanticPointcloud):
    ...
    def forward(
        ...
        observations = self.compute_semantics(observations) # self.compute_semantics: PredictSemantics()其实就是rednet
