...
def setup_keywords_inputs_from_obs_dict(observations_dict: dict):
    ...
    # observations = setup_observations(observations_dict)
    observations = {
        'instruction_text': observations_dict['instruction_text_for_keywords'],
        'semantics': semantics,
    ...
    # for i in range(36):
    for i in range(12):
        if 'rgb_{}'.format(i) not in observations_dict:
            continue
        observations['rgb_{}'.format(i)] = channel_first_representation(observations_dict['rgb_{}'.format(i)])
    # assert 'rgb_12' not in observations_dict

    episodes_info = EpisodesInfo(
        observations_dict["not_done_masks"], # 这个标记的是tour是否结束
        observations_dict["env_name"],
    )
