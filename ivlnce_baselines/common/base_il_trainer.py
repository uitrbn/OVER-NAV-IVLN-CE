            ...
            for k, v in batch.items():
                try:
                    if k == 'instruction_text_for_keywords':
                        continue
                    elif k in ['keywords', 'keywords_map', 'rel_heading_feats']:
                        if k == 'keywords_map':
                            for sub_k in batch[k]:
                                batch[k][sub_k] = [v[sub_k][i] for i in state_index]
                        elif k in ['keywords', 'rel_heading_feats']:
                            batch[k] = [v[i] for i in state_index]
                    else:
                        batch[k] = v[state_index]
                except:
                    print('\007')
                    import pdb; pdb.set_trace()
            if 'instruction_text_for_keywords' in batch:
                del batch['instruction_text_for_keywords']
